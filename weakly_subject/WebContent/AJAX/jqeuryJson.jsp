	...
	window.addEventListener("load", function(){
			...
			var url ="../ajaxBoardList.do"
				...
				var callback  = function(boardList){
									for(i=0; i<boardList.length; i++){
										result.innerHTML += boardList[i].first_name+"<br>";
									}
								}
		...
		btnOne.addEventListener("click",function(){
			...
			var callback = function(board, status, xhr){
				...
				console.log(status);
				console.log(xhr);
				result.innerHTML += board.first_name+"<br>";
			}
			$.getJSON(url,data,callback);
		});
	});
...
<input type="button" id="btnOne">번호로 조회</button>
<input type="button" id="btnList">전체조회</button>
<div id="result"></div>
</body>
</html>